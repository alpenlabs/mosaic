...
pub fn interpolate(known_shares: &[Share]) -> Result<Vec<Share>, Error> {
    ...
    // Commenting out the block below because we need to interpolate using opened output shares,
    // which doesn't include reserved index, in the try_reveal_secret step of Withdrawal
...
mod tests {
    ...
    fn test_interpolate_missing_shares_errors() {
        ...
        assert!(
            !result.is_err(),
            "shouldn't panic if reserved index isn't present"
        );
    }
}
