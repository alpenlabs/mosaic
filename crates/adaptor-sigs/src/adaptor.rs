...
impl Signature {
    ...
    /// is_valid keypair
    pub fn get_pubkey(sk: &ark_secp256k1::Fr) -> Option<ark_secp256k1::Projective> {
        let pk = gen_mul(sk);
        let pk_affine = pk.into_affine();
        let y_is_odd = pk_affine.y.into_bigint().is_odd();
        if y_is_odd {
...
/// Adaptor for the VSSS
#[derive(Debug, Clone, PartialEq, Eq, Copy, CanonicalSerialize, CanonicalDeserialize)]
pub struct Adaptor {
...
mod tests {
    ...
    fn zero_share_commitment_behaves_like_plain_r_prime() {
        ...
        let (x, P) = Signature::keypair(&mut rng);
