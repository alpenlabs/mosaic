...
pub fn interpolate(known_shares: &[Share]) -> Result<Vec<Share>, Error> {
    ...
    let has_reserved = known_shares.iter().any(|share| share.index() == Index::reserved());
    if !has_reserved {
...
fn lagrange_interpolate_whole_polynomial(
    ...
    missing_points
        ...
        .map(|x| {
            ...
            lagrange_coeffs.iter().fold(Scalar::zero(), |acc, (i, coeff_i)| {
                let diff_inv = if i < x {
                    inv[x - i]
                } else {
                    -inv[i - x]
...
mod tests {
    ...
    use crate::constants::{N_COEFFICIENTS, N_CIRCUITS};
    use rand::rngs::OsRng;
    use crate::polynomial::{Polynomial, Share, Index};
    ...
    fn test_interpolate_missing_shares() {
        ...
        let missing_shares = interpolate(&known_shares)
        ...
        let known_indices: std::collections::HashSet<usize> = selected_indices.iter().copied().collect();

        for share in &missing_shares {
            let idx = share.index().get();
            assert!(!known_indices.contains(&idx), "missing share should not be in known shares");
            assert_eq!(share.value(), all_shares[idx].value(), "mismatch at index {idx}");
        }
    }
    ...
    fn test_interpolate_missing_shares_errors() {
        ...
        assert!(matches!(result.unwrap_err(), Error::InvalidShareCount { .. }));
