...
mod values {
    // Full circuit size. This MUST have Log2(C(N_CIRCUITS, N_OPEN_CIRCUITS)) >= 40 to meet our
    // 40-bit security target.
...
mod values {
    // Smaller circuit sizes for tests. These are NOT secure and should never be used outside
    // testing.
